...
class AudioProcessor:
    ...
    def __init__(self, connection, sample_rate=24000):
        ...
        self.chunk_size = int(sample_rate * 0.05) # 50ms chunks i.e., 0.05 
...
class BasicVoiceAssistant:
    ...
    def __init__(
        ...
        input_audio_language: Optional[str] = None,
    ):
        ...
        self.input_audio_language = input_audio_language
        ...
        # Latency tracking
        self._user_speech_stopped_time: Optional[float] = None
        self._first_audio_response_time: Optional[float] = None
        self._latency_logged_for_turn = False
        self._latency_measurements = []
        
        # Conversation transcript tracking
        self._transcript_log = []  # Clean transcript of user/assistant turns
        self._current_assistant_audio_transcript = ""  # For audio transcript
    ...
    def _save_conversation_history(self):
        ...
        if not self.conversation_history and not self._latency_measurements and not self._transcript_log:
            return
        ...
        try:
            ...
            if self._latency_measurements:
                ...
                latency_stats = {
                    ...
                    "measurements": self._latency_measurements
                }
                print(f"\n📊 Latency Stats: Min={latency_stats['min_ms']}ms, Max={latency_stats['max_ms']}ms, Avg={latency_stats['avg_ms']}ms")
            
            history_data = {
                "transcript": self._transcript_log,  # Clean conversation transcript
            ...
            # Also save a plain text transcript for easy reading
            if self._transcript_log:
                transcript_file = f'logs/{timestamp}_transcript.txt'
    ...
    async def _setup_session(self):
        ...
        if self.vad_type == "azure_multilingual_semantic_vad":
            # Azure Multilingual Semantic VAD for Hindi/Kannada/etc
            vad_kwargs = {
            ...
            turn_detection_config = AzureSemanticVadMultilingual(**vad_kwargs)
            logger.info(f"Using azure_multilingual_semantic_vad with threshold={self.vad_threshold}")
        elif self.vad_type == "azure_semantic_vad":
            ...
            logger.info(f"Using azure_semantic_vad with threshold={self.vad_threshold}")
        else:
            # Standard Server VAD
        ...
        # Configure input audio transcription for multilingual support
        # This enables proper transcription for Hindi, Kannada, and other languages
        input_transcription_config = None
        if self.input_audio_transcription_model:
            transcription_kwargs = {"model": self.input_audio_transcription_model}
            if self.input_audio_language:
                transcription_kwargs["language"] = self.input_audio_language
            input_transcription_config = AudioInputTranscriptionOptions(**transcription_kwargs)
            logger.info(f"Input audio transcription: model={self.input_audio_transcription_model}, language={self.input_audio_language or 'auto-detect'}")
    ...
    async def _handle_event(self, event):
        ...
        if event.type == ServerEventType.SESSION_UPDATED:
            ...
            self._user_speech_stopped_time = asyncio.get_event_loop().time()
            self._latency_logged_for_turn = False
            logger.info("🎤 User stopped speaking at %.3f", self._user_speech_stopped_time)
            print("🤔 Processing...")

        elif event.type == ServerEventType.INPUT_AUDIO_BUFFER_COMMITTED:
            # Audio buffer committed - transcript will come via CONVERSATION_ITEM_CREATED event
            logger.debug("Input audio buffer committed")

        elif event.type == ServerEventType.RESPONSE_CREATED:
            logger.info("🤖 Assistant response created at %.3f", asyncio.get_event_loop().time())
            self._active_response = True
            self._response_api_done = False
            self._current_assistant_response = ""
            self._current_assistant_audio_transcript = ""  # Reset audio transcript
            ...
            # Capture assistant's audio transcript as it's generated (what the assistant is saying)
            if hasattr(event, 'delta') and event.delta:
                self._current_assistant_audio_transcript += event.delta

        elif event.type == ServerEventType.RESPONSE_AUDIO_TRANSCRIPT_DONE:
            # Log complete assistant audio transcript and add to transcript log
            if self._current_assistant_audio_transcript:
                logger.info(f"Assistant audio transcript: {self._current_assistant_audio_transcript}")
                
                # Add to transcript log (prefer audio transcript as it's what was actually spoken)
                self._transcript_log.append({
                    "role": "assistant",
                    "text": self._current_assistant_audio_transcript,
                    "timestamp": datetime.now().isoformat()
                })
            elif self._current_assistant_response:
                # Fallback to text response if no audio transcript
                self._transcript_log.append({
                    "role": "assistant",
                    "text": self._current_assistant_response,
                    "timestamp": datetime.now().isoformat()
                })

        elif event.type == ServerEventType.RESPONSE_AUDIO_DELTA:
            # Calculate latency on first audio chunk
            if self._user_speech_stopped_time and not self._latency_logged_for_turn:
                self._first_audio_response_time = asyncio.get_event_loop().time()
                latency_ms = (self._first_audio_response_time - self._user_speech_stopped_time) * 1000
                
                # Store measurement
                latency_record = {
                    "timestamp": datetime.now().isoformat(),
                    "user_speech_stopped": self._user_speech_stopped_time,
                    "first_audio_received": self._first_audio_response_time,
                    "latency_ms": round(latency_ms, 2)
                }
                self._latency_measurements.append(latency_record)
                
                logger.info("⏱️ Response latency: %.2f ms (user stopped -> first audio)", latency_ms)
                print(f"⏱️ Latency: {latency_ms:.2f} ms")
                self._latency_logged_for_turn = True
...
def parse_arguments():
    ...
    # ============================================
    # Input Audio Transcription (Multilingual Support)
    # ============================================
    parser.add_argument(
        "--input-audio-transcription-model",
        help="Model for input audio transcription (e.g., whisper-1 supports 100+ languages). Leave empty to use default.",
        type=str,
        default=os.environ.get("AZURE_VOICELIVE_INPUT_TRANSCRIPTION_MODEL", None),
    )

    parser.add_argument(
        "--input-audio-language",
        help="Language code for transcription (e.g., hi for Hindi, kn for Kannada). Leave empty for auto-detect.",
        type=str,
        default=os.environ.get("AZURE_VOICELIVE_INPUT_LANGUAGE", None),
    )
    ...
    parser.add_argument(
        "--use-api-key", 
        help="Use API key authentication instead of DefaultAzureCredential (Entra ID)", 
        action="store_true", 
        default=False
    )
...
def main():
    ...
    # Default: Use DefaultAzureCredential (Entra ID) - API key only if --use-api-key flag is set
    credential: Union[AzureKeyCredential, AsyncTokenCredential]
    if args.use_api_key and args.api_key:
        # Only use API key if explicitly requested with --use-api-key flag
        credential = AzureKeyCredential(args.api_key)
        logger.info("Using API key credential")
        print("🔑 Using API key authentication...")
    else:
        # Default: Use DefaultAzureCredential for Entra ID authentication
        # This tries multiple auth methods: Azure CLI, VS Code, Managed Identity, etc.
        credential = DefaultAzureCredential()
        logger.info("Using DefaultAzureCredential (Entra ID authentication)")
    ...
    assistant = BasicVoiceAssistant(
        ...
        input_audio_transcription_model=args.input_audio_transcription_model,
        input_audio_language=args.input_audio_language,
    )
